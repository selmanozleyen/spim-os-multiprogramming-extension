...
filename0: .asciiz "LinearSearch.s"
filename1: .asciiz "BinarySearch.s"
filename2: .asciiz "Collatz.s"
filename3: .asciiz "Palindrome.s"
...
main:
    ...
    # rand()
    syscall
    #t0,t1 and t3 will be filled accordingly
    beq $v0,0,load_1_2_3 # here t0 = file1 t1 = file2 t2 =file3
    beq $v0,1,load_0_2_3 # here t0 = file0 t1 = file2 t2 =file3
    beq $v0,2,load_0_1_3 # here t0 = file0 t1 = file1 t2 =file3
    beq $v0,3,load_0_1_2 # here t0 = file0 t1 = file1 t2 =file2

    # when file loading ends
load_end:

    li $t6,3 # load number of times to create process each file
    # loop to load the processes
l2:
    beq $t6,$0,end_l2 # if(t6 == 0) end loop
    ...
    # if v0 == 0 then call execv with $t0
    la $a0,($t0) # $a0 = $t0
    ...
    # if v0 == 0 then call execv with $t1
    la $a0,($t1) # $a0 = $t1
    beq $v0,$0,is_child # if(child) execv 
    ...
    j l2 # jump for the loop 

    # will come there if the loop finishes
end_l2:

    # looping until all the chilren are reaped
l1:
    # while(there_are_children) wait()
    li $v0,20
    # wait()
    syscall
    # if -1 it is finished
    beq $v0,-1,finished # if finished branch
...
# load the given files exept 3 
load_0_1_2:
    # loading file names
    la $t0,filename0
    la $t1,filename1
    la $t2,filename2
    # return after loading the names
    j load_end
# load the given files exept 2
load_0_1_3:
    # loading file names
    la $t0,filename0
    la $t1,filename1
...
# will come here if process is a child
is_child:
    # set for execv
    li $v0,19
    # execv
